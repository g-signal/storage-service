...
abstract class Table {
   ...
   * */
  CompletableFuture<Boolean> setIfValueOrEmpty(ByteString rowId, String columnFamily, String columnName, String columnEquals, Mutation mutation) {
    return setIfValue(rowId, columnFamily, columnName, columnEquals, mutation)
        .thenCompose(mutated -> mutated
            ? CompletableFuture.completedFuture(true)
            : setIfEmpty(rowId, columnFamily, columnName, mutation));
  }
   ...
   * @return a future that yields {@code true} if the identified row was modified or {@code false} otherwise
   */
  CompletableFuture<Boolean> setIfValue(ByteString rowId, String columnFamily, String columnName, String columnEquals, Mutation mutation) {
   ...
   * @return a future that yields {@code true} if the identified row was modified or {@code false} otherwise
   */
  CompletableFuture<Boolean> setIfEmpty(ByteString rowId, String columnFamily, String columnName, Mutation mutation) {
    return toFuture(client.checkAndMutateRowAsync(ConditionalRowMutation.create(tableId, rowId)
        .condition(FILTERS.chain()
            .filter(FILTERS.family().exactMatch(columnFamily))
            .filter(FILTERS.qualifier().exactMatch(columnName))
  ...
  static <T> CompletableFuture<T> toFuture(ApiFuture<T> future) {
