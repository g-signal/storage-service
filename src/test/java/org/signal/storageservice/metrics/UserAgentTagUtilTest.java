...
class UserAgentTagUtilTest {
  ...
  @Test
  void getClientVersionTag() {
    final Map<ClientPlatform, Set<Semver>> recognizedClientVersions =
        Map.of(ClientPlatform.ANDROID, Set.of(new Semver("1.2.3")));

    assertEquals(Optional.of(Tag.of(UserAgentTagUtil.VERSION_TAG, "1.2.3")),
        UserAgentTagUtil.getClientVersionTag("Signal-Android/1.2.3 (Android 4.3)", recognizedClientVersions));

    assertEquals(Optional.empty(),
        UserAgentTagUtil.getClientVersionTag("Signal-Android/1.2.4 (Android 4.3)", recognizedClientVersions));

    assertEquals(Optional.empty(),
        UserAgentTagUtil.getClientVersionTag("Signal-Desktop/1.22.45-foo-0", recognizedClientVersions));
  }
}
